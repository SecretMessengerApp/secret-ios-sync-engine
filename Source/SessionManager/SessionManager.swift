...
@objcMembers public class SessionManager : NSObject, SessionManagerType, UserSessionSource {
    ...
    fileprivate func activateSession(for account: Account, completion: @escaping (ZMUserSession) -> Void) {
        self.withSession(for: account) { [weak self] session in
            guard let self = self else {return}
            ...
            self.userObserverToken = UserChangeInfo.add(userObserver: self, for: user, userSession: session)
            self.initialSyncToken = ZMUserSession.addInitialSyncCompletionObserver(self, context: session.managedObjectContext)
...
extension SessionManager: ZMInitialSyncCompletionObserver {
    public func initialSyncCompleted() {
        guard let manageContext = self.activeUserSession?.managedObjectContext else {
            return
        }
        self.updateCurrentAccount(in: manageContext)
    }
}
